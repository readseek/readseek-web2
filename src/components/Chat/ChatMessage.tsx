    ...
    return (
        <div
            ...
            <div className="relative m-auto flex gap-4 p-4 text-base md:max-w-2xl md:gap-6 md:py-6 lg:max-w-2xl lg:px-0 xl:max-w-3xl">
                <div className="min-w-[40px] font-bold">{message.role === 'assistant' ? 'AI' : 'You'}:</div>
                    ...
                    {message.role === 'user' ? (
                        <div className="flex w-full">
                            {isEditing ? (
                                <div className="flex w-full flex-col">
                                    ...
                                    <div className="mt-10 flex justify-center space-x-4">
                                        <button className="h-[40px] rounded-md bg-blue-500 px-4 py-1 text-sm font-medium text-white enabled:hover:bg-blue-600 disabled:opacity-50" onClick={handleEditMessage} disabled={messageContent.trim().length <= 0}>
                                            {'Save & Submit'}
                                        </button>
                                        <button
                            ...
                            {(isHovering || window.innerWidth < 640) && !isEditing && (
                                <button className={`absolute ${window.innerWidth < 640 ? 'bottom-1 right-3' : 'right-[-20px] top-[26px]'}`}>
                                    <IconEdit size={20} className="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300" onClick={toggleEditing} />
                                </button>
                            )}
                        </div>
                    ) : (
                        <>
                            ...
                            {(isHovering || window.innerWidth < 640) && <CopyButton messagedCopied={messagedCopied} copyOnClick={copyOnClick} />}
                        </>
                    )}
                </div>
            </div>
