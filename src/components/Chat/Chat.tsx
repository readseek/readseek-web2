...
export const Chat: FC<Props> = memo(({ conversation, keyConfiguration, messageIsStreaming, loading, onSend, onUpdateConversation, onEditMessage, stopConversationRef, handleKeyConfigurationValidation }) => {
                ...
                <div className="max-h-full overflow-x-hidden" ref={chatContainerRef}>
                    {conversation.index?.indexName.length === 0 && conversation.messages.length === 0 ? (
                        <>
                            {!isUploadSuccess ? (
                                <>
                                    <div id="alert-2" className="mb-4 flex rounded-lg bg-red-50 p-4 text-red-800 dark:bg-gray-800 dark:text-red-400" role="alert">
                                        <svg aria-hidden="true" className="h-5 w-5 shrink-0" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <div className="mx-auto flex w-[350px] flex-col space-y-10 pt-12 sm:w-[600px]">
                                <div className="flex h-full flex-col space-y-4 rounded border border-neutral-200 p-4 dark:border-neutral-600">
                                    ...
                                    {CHAT_FILES_MAX_SIZE != 0 && (
                                        <>
                                            <p className="mt-2 px-8 text-xs text-gray-500 dark:text-gray-400">This environment is only for trial and supports a maximum file size of {validFileSize(CHAT_FILES_MAX_SIZE)}.</p>
                                            <p className="mt-2 px-8 text-xs text-gray-500 dark:text-gray-400">
                                                Here are some good starting questions:
                                                <a className="text-xs text-gray-500 underline dark:text-gray-400" href="https://github.com/guangzhengli/ChatFiles/blob/main/doc/Example.md">
                                                    {' '}
                                                    Good Examples .
                                                </a>
                                            </p>
                                            <p className="mt-2 px-8 text-xs text-gray-500 dark:text-gray-400">
                                                If you need to upload larger files, please deploy your own chatfiles by:
                                                <a className="text-xs text-gray-500 underline dark:text-gray-400" href="https://github.com/guangzhengli/ChatFiles">
                                                    {' '}
                                                    ChatFiles
                                                </a>
                                            </p>
                        ...
                        <>
                            <div className="flex justify-center border border-b-neutral-300 bg-neutral-100 py-2 text-sm text-neutral-500 dark:border-none dark:bg-[#444654] dark:text-neutral-200">
                                {'File'}: {conversation.index.indexName}
                                <IconClearAll className="ml-2 cursor-pointer hover:opacity-50" onClick={onClearAll} size={18} />
                            </div>
